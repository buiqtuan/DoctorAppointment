...
const getallusers = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: "Unable to get all users", error: error.message });
  }
};
...
const login = async (req, res) => {
  try {
    ...
    // Verify role matches
    if (user.role !== role) {
      return res.status(404).json({ message: "Role does not exist for this account" });
    }
...
const forgotpassword = async (req, res) => {
  try {
    ...
    // Create reset link
    const resetLink = `${process.env.FRONTEND_URL || "https://appointmentdoctor.netlify.app"}/resetpassword/${user.id}/${token}`;
    ...
    transporter.sendMail(mailOptions, (error, info) => {
      if (error) {
        ...
        return res.status(500).json({ status: "Error sending email", error: error.message });
      } else {
